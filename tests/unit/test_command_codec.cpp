...
void test_encode_move_position_mode_2()
{
  ...
  auto data = CommandDecoder::encode_move_position_mode_2(pos1, spd1, acc1);
  ...
  data = CommandDecoder::encode_move_position_mode_2(pos2, spd2, acc2);
  ...
  data = CommandDecoder::encode_move_position_mode_2(pos3, spd3, acc3);
  ...
  data = CommandDecoder::encode_move_position_mode_2(pos4, spd4, acc4);
...
void test_encode_stop_position_mode_2()
{
  ...
  auto data = CommandDecoder::encode_stop_position_mode_2(decel1);
  ...
  data = CommandDecoder::encode_stop_position_mode_2(decel2);
  ASSERT_EQUAL(data[0], 0, "Zero decel (instant stop)");

  Acceleration decel3 = Acceleration::from_internal(255);
  data = CommandDecoder::encode_stop_position_mode_2(decel3);
...
void test_encode_move_speed_mode()
{
  ...
  auto data = CommandDecoder::encode_move_speed_mode(spd1, acc1);
  ...
  Acceleration acc2 = Acceleration::from_internal(100);
  data = CommandDecoder::encode_move_speed_mode(spd2, acc2);
...
void test_encode_config_commands()
{
  ...
  auto data = CommandDecoder::encode_set_working_current(2000);
...
void test_read_encoder_carry()
{
  std::cout << "\n=== read_encoder_carry Tests ===" << std::endl;
  ...
  Position pos = CommandDecoder::read_encoder_carry(data);
  ASSERT_EQUAL(pos.get_ticks(), 86580, "Position from carry=5, value=0x1234");

  // Zero values
  data = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  pos = CommandDecoder::read_encoder_carry(data);
...
void test_read_motor_status()
{
  std::cout << "\n=== read_motor_status Tests ===" << std::endl;

  // STOP state
  std::vector<uint8_t> data = {0x00};
  auto status = CommandDecoder::read_motor_status(data);
  ASSERT_EQUAL(status.state, CommandDecoder::MotorStatus::STOP, "Status = STOP");

  // MOVING state
  data = {0x01};
  status = CommandDecoder::read_motor_status(data);
  ASSERT_EQUAL(status.state, CommandDecoder::MotorStatus::MOVING, "Status = MOVING");

  // HOMING state
  data = {0x02};
  status = CommandDecoder::read_motor_status(data);
  ASSERT_EQUAL(status.state, CommandDecoder::MotorStatus::HOMING, "Status = HOMING");

  // Unknown state (treated as HOMING)
  data = {0x03};
  status = CommandDecoder::read_motor_status(data);
  ASSERT_EQUAL(status.state, CommandDecoder::MotorStatus::HOMING, "Unknown status → HOMING");

  // Empty data
  data.clear();
  status = CommandDecoder::read_motor_status(data);
  ASSERT_EQUAL(status.state, CommandDecoder::MotorStatus::STOP, "Empty data → STOP");
}

void test_read_protection_status()
{
  std::cout << "\n=== read_protection_status Tests ===" << std::endl;

  // Protected state
  std::vector<uint8_t> data = {0x01};
  auto ps = CommandDecoder::read_protection_status(data);
  ASSERT_TRUE(ps.protected_state, "Protected state = true");

  // Not protected
  data = {0x00};
  ps = CommandDecoder::read_protection_status(data);
...
void test_read_homing_status()
{
  std::cout << "\n=== read_homing_status Tests ===" << std::endl;

  std::vector<uint8_t> data;
  CommandDecoder::HomingStatus hs;

  // Test IDLE state
  data = {0x00};
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::IDLE), "Status = IDLE");
  ASSERT_EQUAL(hs.error_code, 0, "IDLE error_code = 0");

  // Test IN_PROGRESS state
  data = {0x01};
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::IN_PROGRESS), "Status = IN_PROGRESS");
  ASSERT_EQUAL(hs.error_code, 0, "IN_PROGRESS error_code = 0");

  // Test COMPLETED state
  data = {0x02};
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::COMPLETED), "Status = COMPLETED");
  ASSERT_EQUAL(hs.error_code, 0, "COMPLETED error_code = 0");

  // Test ERROR state (value 3)
  data = {0x03};
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::ERROR), "Status = ERROR (3)");
  ASSERT_EQUAL(hs.error_code, 3, "ERROR error_code = 3");

  // Test ERROR state (value 4 - no endstop)
  data = {0x04};
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::ERROR), "Status = ERROR (4)");
  ASSERT_EQUAL(hs.error_code, 4, "ERROR error_code = 4");

  // Test ERROR state (value 5 - position error)
  data = {0x05};
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::ERROR), "Status = ERROR (5)");
  ASSERT_EQUAL(hs.error_code, 5, "ERROR error_code = 5");

  // Test empty data (communication error)
  data.clear();
  hs = CommandDecoder::read_homing_status(data);
  ASSERT_EQUAL(static_cast<int>(hs.state), static_cast<int>(CommandDecoder::HomingStatus::ERROR), "Empty data → ERROR");
...
void test_encode_homing_commands()
{
  ...
  // Test encode_set_home_parameters (Commandtype 0x90 - "Set the parameter of home")
  // Format: [hmTrig][hmDir][HmSpeed_hi][HmSpeed_lo][EndLimit]
  auto data = CommandDecoder::encode_set_home_parameters(
  ...
  data = CommandDecoder::encode_set_home_parameters(
  ...
  data = CommandDecoder::encode_set_nolimit_home_parameters(
      reverse_pos,
  ...
  data = CommandDecoder::encode_set_zero_mode(
      CommandDecoder::ZeroMode::NEAR_MODE, // mode=2: NearMode
      CommandDecoder::ZeroModeTask::SET,   // clean_set=SET (set zero)
      ZeroingSpeed::MEDIUM,
      Direction::CW);
  ...
  data = CommandDecoder::encode_set_zero_mode(
      CommandDecoder::ZeroMode::DIR_MODE,  // mode=1: DirMode
      CommandDecoder::ZeroModeTask::CLEAN, // clean_set=CLEAN (clean zero)
  ...
  data = CommandDecoder::encode_set_zero_mode(
      CommandDecoder::ZeroMode::DIR_MODE, // mode=1: DirMode
      CommandDecoder::ZeroModeTask::SET,  // clean_set=SET
  ...
  data = CommandDecoder::encode_set_zero_mode(
      CommandDecoder::ZeroMode::DIR_MODE,  // mode
      CommandDecoder::ZeroModeTask::CLEAN, // clean_set=CLEAN (clean zero)
      ZeroingSpeed::SLOW,
      Direction::CW);
...
void test_roundtrip_encoding()
{
  ...
  auto encoded = CommandDecoder::encode_move_position_mode_2(pos_obj, spd_obj, acc_obj);
  ...
  Speed decoded_speed = CommandDecoder::read_current_speed(speed_data);
  ...
  Position decoded_pulses = CommandDecoder::read_pulse_count(pulse_data);
...
int main()
{
  ...
  test_read_encoder_carry();
  test_read_motor_status();
  test_read_protection_status();
  test_read_homing_status();
